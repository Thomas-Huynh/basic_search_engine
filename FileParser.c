...
char* ReadFileToString(const char *file_name, int *size) {
  ...
  if (stat(file_name, &file_stat) == -1) {
    // error
    fprintf(stderr, "Could not find %s", file_name);
    return NULL;
  }
  ...
  if (!S_ISREG(file_stat.st_mode)) {
    fprintf(stderr, "%s is not a regular file", file_name);
    return NULL;
  }
  ...
  while (left_to_read > 0) {
    ...
    if (result == -1) {
      if (errno != EAGAIN && errno != EINTR) {
        // real error, exit the program
        fprintf(stderr, "Error when reading %s", file_name);
        close(fd);
        return NULL;
      } else {
...
static void AddWordPosition(HashTable *tab, char *word,
  ...
  if (HashTable_Find(tab, hash_key, &kv)) {
    ...
    wp = (WordPositions*) malloc(sizeof(WordPositions) * 1);

    //allocates a copy of the word onto the heap
    char* alloc_word = (char*) malloc(sizeof(char) * (strlen(word) + 1));
    snprintf(alloc_word, strlen(word) + 1, "%s", word);

    wp->word = alloc_word;
    wp->positions = LinkedList_Allocate();
    LinkedList_Append(wp->positions, (LLPayload_t) (int64_t) pos);

    HTKeyValue_t payload = {hash_key, (HTValue_t) wp};
    HashTable_Insert(tab, payload, &kv);
  }
}
