...
// Finds the keyvalue based on a given key at the given chain
// and removes it if requested
// Arguments:
// chain: The linked list that will be searched
// keyvalue: A pointer given by the caller that will
//  hold the keyvalue (if it exists)
// key: The key used to search for the keyvalue
// remove: Signals whether or not the caller wants the key to be removed
//
// Returns: Whether or not the key exists (before removal) in the chain
static bool FindChainKeyMaybeRemove(LinkedList* chain, HTKeyValue_t* keyvalue,
                                     HTKey_t key, bool remove);
                                                                          ///////////////////////////////////////////////////////////////////////////////
...
static bool FindChainKeyMaybeRemove(LinkedList* chain, HTKeyValue_t* keyvalue,
  ...
  // Iterates through chain to look for matching keys (or end of chain)
  HTKeyValue_t* keyvalue_ptr = NULL;
  LLIterator_Get(iter, (LLPayload_t*)(&keyvalue_ptr));
  while (keyvalue_ptr->key != key && LLIterator_IsValid(iter)) {
    LLIterator_Get(iter, (LLPayload_t*)(&keyvalue_ptr));
    if (keyvalue_ptr->key != key) {
      LLIterator_Next(iter);
    }
  }

  // Final check to see if key is truly not in the chain
  if (!LLIterator_IsValid(iter)) {
    LLIterator_Free(iter);
    return false;
  }

  *keyvalue = *keyvalue_ptr;
  if (remove) {
...
bool HashTable_Insert(HashTable *table,
  ...
  HTKeyValue_t* new_payload = (HTKeyValue_t*) malloc(sizeof(newkeyvalue));
  *new_payload = newkeyvalue;

  bool in_table = FindChainKeyMaybeRemove(chain, oldkeyvalue,
                                          newkeyvalue.key, true);
  if (!in_table) {
    LinkedList_Push(chain, new_payload);
    table->num_elements++;
  } else {
    LinkedList_Push(chain, new_payload);
  }
...
bool HashTable_Find(HashTable *table,
  ...
  bool in_table = FindChainKeyMaybeRemove(chain, keyvalue, key, false);
  return in_table;
}
...
bool HashTable_Remove(HashTable *table,
  ...
  bool in_table = FindChainKeyMaybeRemove(chain, keyvalue, key, true);
  if (in_table) {
    table->num_elements--;
  }
...
bool HTIterator_IsValid(HTIterator *iter) {
  ...
  // STEP 4: implement HIterator_IsValid.
...
bool HTIterator_Next(HTIterator *iter) {
  ...
  // Frees old bucket's iterator and keeps attempting to
  // create a bucket iterator
  // for each row
  while (iter->bucket_idx + 1 < iter->ht->num_buckets &&
  ...
  // gone through all possible buckets and HT Iterator is now invalid
...
bool HTIterator_Get(HTIterator *iter, HTKeyValue_t *keyvalue) {
  ...
  if (!HTIterator_IsValid(iter)) {
    ...
    HTKeyValue_t* keyvalue_ptr;
    LLIterator_Get(iter->bucket_it, (LLPayload_t*)(&keyvalue_ptr));
    *keyvalue = *keyvalue_ptr;
    return true;
  }
}
