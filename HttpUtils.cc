...
bool IsPathSafe(const string &root_dir, const string &test_file) {
  ...
  char buf1[PATH_MAX];
  char buf2[PATH_MAX];
  realpath(root_dir.c_str(), buf1);
  string absolute_root = string(buf1);

  realpath(test_file.c_str(), buf2);
  string absolute_path = string(buf2);
  return absolute_path.find(absolute_root + "/") != string::npos;
