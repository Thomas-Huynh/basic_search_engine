...
bool LinkedList_Pop(LinkedList *list, LLPayload_t *payload_ptr) {
  ...
  if (list->num_elements == 1) {
    ...
    *payload_ptr = list->head->payload;
    ...
    // General case: List has at least 2 elements
    *payload_ptr = list->head->payload;
...
void LinkedList_Append(LinkedList *list, LLPayload_t payload) {
  ...
  if (list->num_elements == 0) {
    ...
    // General case; list has at least 1 element
    ln->prev = list->tail;
    list->tail->next = ln;
    list->tail = ln;
    list->tail->next = NULL;
...
bool LLIterator_Remove(LLIterator *iter,
  ...
  if (iter->list->num_elements <= 1) {
    // Degenerate case: List becomse empty post-deletion
  ...
  if (iter->node == iter->list->head) {
    // Degenerate case: Deleting list head
    ...
    // Degenreate case: Deleting list tail
...
bool LLSlice(LinkedList *list, LLPayload_t *payload_ptr) {
  ...
  if (list->num_elements == 1) {
    // Degenerate case: List becomes empty post-slice
