...
void MemIndex_AddPostingList(MemIndex *index, char *word, DocID_t doc_id,
  ...
  if (!HashTable_Find(index, key, &mi_kv)) {
    ...
    wp = (WordPostings*) malloc(sizeof(WordPostings) * 1);
    wp->word = word;
  ...
  HTKeyValue_t entry = {(HTKey_t) doc_id, (HTValue_t) postings};
  HashTable_Insert(wp->postings, entry, &unused);
}
...
LinkedList* MemIndex_Search(MemIndex *index, char *query[], int query_len) {
  ...
  // finds appropriate word according to query
  ...
  if (HashTable_Find(index, key, &kv)) {
    ...
    // not in the table, return null
    LinkedList_Free(ret_list, (LLPayloadFreeFnPtr)free);
    return NULL;
  }


  // iterates through wp->postings, gets each doc's associated linkedlist
  // makes SearchResult struct, and adds it to ret_list
  HTIterator* iter = HTIterator_Allocate(wp->postings);
  while (HTIterator_IsValid(iter)) {
    // extract keyvalue from WordPostings struct
    HTIterator_Get(iter, &kv);
    DocID_t doc_id = (DocID_t) kv.key;
    LinkedList* doc_list = (LinkedList*) kv.value;

    // initialize and add SearchResult struct
    SearchResult* res = (SearchResult*) malloc(sizeof(SearchResult) * 1);
    res->doc_id = doc_id;
    res->rank = LinkedList_NumElements(doc_list);
  ...
  for (i = 1; i < query_len; i++) {
    ...
    HTKeyValue_t kv;
    if (!HashTable_Find(index, FNVHash64((unsigned char*) query[i],
        strlen(query[i])), &kv)) {
      LinkedList_Free(ret_list, (LLPayloadFreeFnPtr)free);
      return NULL;
    }
    ...
    for (j = 0; j < num_docs; j++) {
      LLPayload_t ll_payload;
      LLIterator_Get(ll_it, &ll_payload);

      SearchResult* res = (SearchResult*) ll_payload;
      if (HashTable_Find(wp->postings, res->doc_id, &kv)) {
        // found the thing
        LinkedList* res_list = (LinkedList*) kv.value;
        res->rank += LinkedList_NumElements(res_list);
