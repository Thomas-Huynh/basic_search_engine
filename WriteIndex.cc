...
static int WriteHeader(FILE *f, int doctable_bytes, int memidx_bytes) {
  ...
  for (int i = 0; i < doctable_bytes + memidx_bytes; i++) {
    char curr_byte;
    if (fread(&curr_byte, sizeof(char), 1, f) != 1) {
      return kFailedWrite;
    }
    crc.FoldByteIntoCRC(curr_byte);
  }
...
static int WriteHashTable(FILE *f, IndexFileOffset_t offset, HashTable *ht,
  ...
  for (int i = 0; i < ht->num_buckets; i++) {
    ...
    record_pos += WriteHTBucketRecord(f, record_pos, bucket_size, bucket_pos);
    bucket_pos += WriteHTBucket(f, bucket_pos, ht->buckets[i], fn);
  }
...
static int WriteHTBucketRecord(FILE *f,
  ...
  BucketRecord br = BucketRecord(num_elts, bucket_offset);
  br.ToDiskFormat();
  ...
  // STEP 6.
  // Write the BucketRecord.
  fwrite(&br, sizeof(BucketRecord), 1, f);
...
static int WriteHTBucket(FILE *f, IndexFileOffset_t offset, LinkedList *li,
  ...
  for (int i = 0; i < num_elts; i++) {
    ...
    ElementPositionRecord epr = ElementPositionRecord(element_pos);
    epr.ToDiskFormat();
    fwrite(&epr, sizeof(ElementPositionRecord), 1, f);
    ...
    HTKeyValue_t* elem = reinterpret_cast<HTKeyValue_t*>(kv);
    int num_written = fn(f, element_pos, elem);

    // Advance to the next element in the chain, updating our offsets.
    record_pos += sizeof(ElementPositionRecord);
    element_pos += num_written;  // you may need to change this logic
    LLIterator_Next(it);
  }
  LLIterator_Free(it);
...
static int WriteDocidToDocnameFn(FILE *f, IndexFileOffset_t offset,
  ...
  int16_t file_name_bytes = sizeof(kv->value);
...
static int WriteDocIDToPositionListFn(FILE *f,
  ...
  for (int i = 0; i < num_positions; i++) {
    ...
    position = DocIDElementPosition(reinterpret_cast<uintptr_t>(payload));
    position.ToDiskFormat();
    fwrite(&position, sizeof(DocIDElementPosition), 1, f);
